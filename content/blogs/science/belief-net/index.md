...
- Classical HMMs: Use the Baum-Welch (EM) algorithm. 
...
> We asked ourselves: *Can we train an HMM exactly like a Transformer?.*

### Our Solution: Belief Net

**Belief Net** is a framework that formulates the HMM's recursive "forward filter" as a structured neural network. 
...
We put Belief Net to the test against classical baselines and modern Transformer architectures:
...
{{ image(path="img/hmm-training.png", width=1000, alt="HMM Training Results") }}

### Task 2. Real-World Language Data

When tested on the Federalist Papers dataset, Belief Net achieved lower perplexity than classical HMM methods. 
While Transformers (unsurprisingly) still lead in raw predictive power, Belief Net provides something they can't: **Visualizable Logic**.


{{ image(path="img/text-training.png", width=3000, alt="HMM Training Results") }}
